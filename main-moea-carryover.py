...
def wrapper(vars):
  ...
  return([-results.SHA_storage.resample('AS-OCT').last().sum()  - results.ORO_storage.resample('AS-OCT').last().sum() - results.FOL_storage.resample('AS-OCT').last().sum()])
...
algorithm.run(1000)
pickle.dump(algorithm, open("pickle_files/carryover_results%s.pkl"%seed, "wb" ))
