...
class ORCA_wrapper(Problem):
    ...
    def evaluate(self, solution):
      ...
      for key,df in df_dict.items():
        model = Model(df,dfhist,SHA_shift, ORO_shift, FOL_shift,SHA_exceedance,ORO_exceedance,FOL_exceedance,SHA_carryover,ORO_carryover,FOL_carryover,wyt_exc,sd='10-01-1999',projection = True, sim_gains = True) #climate scenario test
...
if __name__ == "__main__":
  ...
  with PoolEvaluator(pool) as evaluator:
      algorithm = NSGAIII(problem, {"divisions_outer":24},evaluator=evaluator)
      algorithm.run(100)
  int1 =  Integer(-60,60)
  results_list = []
  variable_list = []
  ...
  pickle.dump(results_list, open("pickle_files/objective_all-20000-30test.pkl", "wb" ))
  pickle.dump(variable_list, open("pickle_files/vriable_all-20000-30test.pkl", "wb" ))
