...
#         'SHA_fci', 'ORO_fci', 'FOL_fci', 'BND_swe', 'ORO_swe', 'YRS_swe', 'FOL_swe']
input_ids = ['SR_WYI','SR_WYT_rolling','SR_WYT', 'WYT_sim','WYI_sim']
...
for index_exceedence_sac in [6,7,9,10,11,12,13,14]:
	...
	# forc_df = pd.read_csv('orca/data/scenario_runs/%s/orca-data-climate-forecasted-%s-excdn_%s.csv'%(s,s), index_col = 0, parse_dates = True)
		...
		# projection_results.to_csv('orca/data/scenario_runs/%s/%s-results-excd_%s.csv'%(s,s,index_exceedence_sac))
		# comm.barrier()
		...
		# 	dfobj = pd.DataFrame()
		# 	for sc in scenarios: 	
		# 		projection_results = pd.read_csv('orca/data/scenario_runs/%s/%s-results-excd_%s.csv'%(sc,sc,index_exceedence_sac), index_col = 0, parse_dates = True)
		# 		dfobj[sc] = projection_results[obj]
		# 	dfobj.to_csv('orca/data/climate_results/%s-excd_%s.csv'%(obj,index_exceedence_sac))
		# comm.barrier()	
	# call(['rm', 'orca/data/scenario_runs/%s/orca-data-climate-forecasted-%s-excdn_%s.csv'%(s,s,index_exceedence_sac)])
		# if comm.rank >= 26 and comm.rank <=30: 
	if comm.rank <= 5:
			...
			for sc in scenarios: 	
				...
				dfobj[sc] = projection_results[obj]
			dfobj.to_csv('orca/data/climate_input_forecasts/%s-excd_%s.csv'%(obj,index_exceedence_sac))
	comm.barrier()	
		# call(['rm', 'orca/data/scenario_runs/%s/%s-results-excd_%s.csv'%(s,s,index_exceedence_sac)])
		# call(['rm', 'orca/data/scenario_runs/%s/%s-results-excd_%s.csv'%(s,s,index_exceedence_sac)])
	call(['rm', 'orca/data/scenario_runs/%s/orca-data-climate-forecasted-%s-excdn_%s.csv'	%(s,s,index_exceedence_sac)])
