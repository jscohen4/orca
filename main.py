...
calc_NSEs = False #True if calculating Nash Sutcliffe Equivalents (only relevant for historical scenario)
plot = True
 #True if plotting outputs, need calc_NSEs to also be true if plotting historical results!!!!
...
sc = 'ccsm20402050' #cmip5 climate scenario to use, if projection = True
...
if not projection:
  model = Model('orca/data/historical_runs_data/orca-data-forecasted.csv', 'orca/data/historical_runs_data/orca-data-forecasted.csv',SHA_shift, ORO_shift, FOL_shift,sd='10-01-1999',projection = False, sim_gains = True) #beacuse of rolling calc in gains, we start on 10th day of
