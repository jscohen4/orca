...
### Section 3.2.1 part 1: Baseline Policy Performance:
  ...
  2. Run `run_all_climate_projections.py` to simulate baseline performance in series. 
  
  2: Alternatively, simulations can be run in parallel, using 97 processors (1 for each scenario), with the script `main-      parallel-base-cc.py`. Use the following command in a shell script: `mpirun -n 97 python main-parallel-base-cc.py`.
