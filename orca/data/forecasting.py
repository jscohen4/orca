...
def get_WRF_forecast_WYI(df, stats_file,index_exceedance_sac,prev): #now determining forecasting regression coefficients based off perfect foresight
	flow_sites = ['BND_fnf', 'ORO_fnf', 'YRS_fnf', 'FOL_fnf']
	snow_sites = ['BND_swe', 'ORO_swe', 'YRS_swe', 'FOL_swe']
...
def WRF_forecast(df,WYI_stats_file,carryover_stats_file,window_type,window_length,index_exceedance_sac):
	if window_type == 'historical':
		WYI_stats = get_WRF_forecast_WYI(df,WYI_stats_file,index_exceedance_sac,7)
		WYI_sim = WYI_stats[0]
		...
		for r, swe, res_id in zip(res_frames, snow_sites, res_ids):
			...
			for i,s in enumerate(carryover_stats):
				...
				v = np.append(yearly_stats,np.tile(yearly_stats, 1)) ##2007-2008 WY
				# v = np.append(v,[yearly_stats[364]]) #leap year
				for y in range(2): #2009-2016 WYs
					v = np.append(v,np.tile(yearly_stats, 4))
					v = np.append(v,[yearly_stats[364]]) #leap year
