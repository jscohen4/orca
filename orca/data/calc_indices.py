...
def find_running_WYI(df,zscore1, zscore2): #water year type was calculated in scraper in master. I think we should determine wyt in a different script, just to keep model.py more concise. df is orca-data.csv
  ...
  remaining_flow = np.zeros((8,num_years))
  monthly_snowpack = np.zeros((8,num_years)) #365 x number of years
  monthly_flow = np.zeros((10,num_years))
  ...
  snow_to_date = np.zeros((8,num_years))
  yearly_flow = np.zeros(num_years)
  ...
  for t in range(0,num_days): 
    ...
    daily_flow = fnf_SAC[t-1] + fnf_FEA[t-1] + fnf_YUB[t-1] + fnf_AME[t-1]
